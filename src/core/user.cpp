...
void User::addMoment(const Moment& moment)
{
    life_.addMoment(moment);
}

void User::removeMoment(uint32_t moment_id)
{
    life_.removeMoment(moment_id);
}

const Moment& User::getMoment(const uint32_t moment_id) const
{
...
void User::changeProfilePicture(const std::string& new_profile_picture_path)
{
    profile_picture_path_ = new_profile_picture_path;
}

const std::string& User::getProfilePicturePath() const
{
